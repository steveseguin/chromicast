...
def main():
    ...
    sub = Popen(["pyinstaller", "--clean", "--onefile", "--windowed", "--icon", "images/chromicast.ico", "pyinstaller.spec"], env=env)
    sub.communicate()
    ...
    cefapp_dir = os.path.join(curdir, "dist", "chromicast")
    executable = os.path.join(cefapp_dir, "chromicast"+EXE_EXT)
    ...
    # run the result binary using "cmd.exe /k chromicast.exe", so that
